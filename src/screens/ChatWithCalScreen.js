...
const extractNumberAfterLabel = (text = '', labels = []) => {
  const normalized = String(text).replace(/,/g, '.');
  for (const label of labels) {
    const match = normalized.match(new RegExp(`${label}\\s*(?:คือ|=|:)?\\s*(\\d+(?:\\.\\d+)?)`, 'i'));
...
const parseProfileFromText = (text = '') => {
  const age = extractNumberAfterLabel(text, ['อายุ', 'age']);
  const height = extractNumberAfterLabel(text, ['ส่วนสูง', 'สูง', 'height']);
  const weight = extractNumberAfterLabel(text, ['น้ำหนัก', 'หนัก', 'weight']);
  const nextData = {};
  ...
  if (ageNum >= 5 && ageNum <= 100) nextData.age = String(ageNum);
  if (heightNum >= 80 && heightNum <= 250) nextData.height = String(heightNum);
  if (weightNum >= 20 && weightNum <= 300) nextData.weight = String(weightNum);

  return nextData;
};

const buildProfileSavedReply = (profileData) => {
  const lines = ['บันทึกข้อมูลส่วนตัวให้แล้วนะ'];
  if (profileData.age) lines.push(`อายุ ${profileData.age} ปี`);
  if (profileData.height) lines.push(`ส่วนสูง ${profileData.height} ซม.`);
  if (profileData.weight) lines.push(`น้ำหนัก ${profileData.weight} กก.`);
  lines.push('');
  lines.push('ระบบจะอัปเดต BMI, เป้าแคลต่อวัน และเป้าโปรตีนตามข้อมูลให้อัตโนมัติ');
  return lines.join('\n');
};
...
async function askCalAI({ prompt, base64Image, userData, stats, summary, todayEntries, mealBreakdown }) {
  if (!API_KEY) {
    return {
      action: 'ai_error',
      reply: 'AI มีปัญหา\n\nสาเหตุ: ยังไม่ได้ตั้งค่า EXPO_PUBLIC_GEMINI_API_KEY ในไฟล์ .env\n\nให้สร้างไฟล์ .env แล้วใส่ Gemini API Key ใหม่ จากนั้นรัน npx expo start -c',
    };
  }
...
`;
  ...
  try {
    ...
    if (!response.ok || data.error) {
      return {
        action: 'ai_error',
        reply: getGeminiErrorMessage(response.status, data),
      };
    }
    ...
    if (!rawText) {
      return {
        action: 'ai_error',
        reply: 'AI มีปัญหา\n\nสาเหตุ: Gemini ไม่ส่งข้อความตอบกลับมา อาจเกิดจากรูปไม่ชัด คำตอบถูกบล็อก หรือ response ว่าง',
      };
    }
    ...
    return {
      action: 'ai_error',
      reply: `AI มีปัญหา\n\nสาเหตุ: วิเคราะห์คำตอบจาก Gemini ไม่สำเร็จ อาจเป็นเพราะอินเทอร์เน็ต/API มีปัญหา หรือ AI ส่ง JSON กลับมาไม่ถูกต้อง\nรายละเอียด: ${error?.message || 'parse/network error'}`,
    };
  }
}
...
export default function ChatWithCalScreen({ navigation }) {
  ...
  const sendMessage = async (text, base64Image = null, imageUri = null) => {
    ...
    if (!base64Image && text) {
      const profileData = parseProfileFromText(text);
      if (Object.keys(profileData).length > 0) {
        ...
        appendMessage({
          id: Date.now() + 1,
          text: buildProfileSavedReply(profileData),
          sender: 'bot',
        });
        return;
      }
    ...
    try {
      ...
      appendMessage({
        id: Date.now() + 1,
        text: `AI มีปัญหา\n\nสาเหตุ: ระบบแชตน้องแคลประมวลผลไม่สำเร็จ อาจเกิดจาก state ในแอพหรือข้อมูลอาหารที่ส่งมาไม่ครบ\nรายละเอียด: ${error?.message || 'unknown app error'}`,
        sender: 'bot',
      });
    } finally {
      setLoading(false);
